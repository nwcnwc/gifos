...
- `name` / `shortName` / `version` — **display identity**. `name` is the full title. `shortName` is a compact label (≤ ~20 chars, e.g. `"Chess"` for `"Chess Grandmaster"`) and `version` a short string (`"1.0"`, `"2.3"`). On a signed app's desktop tile the `shortName` is the **nameplate**, and it takes the place of the filename entirely — the tile carries one name, the one that was signed, rather than repeating an unvouched-for filename underneath a pill written across the artwork. It wraps to a second line rather than losing letters. The `version` is NOT on the plate: it rides the tile's **fresh pill** (the slot that otherwise says `NEW`) and disappears with it the moment the app has saved data — a version is worth reading once, when a copy arrives, and is noise afterwards. It stays readable forever on the nameplate's tooltip, alongside the filename. Both appear again in the app's runtime header. **All of this only when the app is signed**: an unsigned GIF can claim any name or version, so its tile shows its filename and a plain `NEW`. A nameplate's presence means "this signed author declares this is Chess."
...
- `capabilities.links` — **open a tapped link in a new tab**. A sandboxed frame cannot `window.open` or honour `target="_blank"` until the parent adds `allow-popups`, and even then the new window inherits the sandbox unless `allow-popups-to-escape-sandbox` is set too. Declaring `links` adds both tokens (`runtime.js`). The app still cannot fetch (`connect-src 'none'`) and cannot navigate this window (no `allow-top-navigation`). Shown in Abilities, revocable. A map pin or a citation is the shape; it is not a network path.
...
- `capabilities.wasm` — **the WASM hatch**. A sandboxed app normally *cannot run WebAssembly*: the default app CSP has no `'wasm-unsafe-eval'` (Chrome refuses WASM under a bare `script-src 'unsafe-inline'`) and no `worker-src` (workers are blocked). Declaring `wasm` swaps in a policy relaxed by **exactly three things and nothing else**: `'wasm-unsafe-eval'` added to `script-src` (a module can instantiate), `worker-src blob:` (a heavy engine can run on a Web Worker), and `connect-src blob: data:` (emscripten-style loaders may `fetch()` wasm/worker code the app minted from its *own bytes* as blob:/data: URLs — same-process plumbing that carries no origin and touches no wire). **The network stays unreachable** — no host can appear in a blob: or data: URL, so the engine and any worker still get *zero* connectivity; an app carries its engine inside itself (base64 or the install-time asset cache) and instantiates from bytes. Same airtight sandbox, just allowed to *compute*; the launch acknowledgement says the app "runs a compiled engine on your device." This is what lets an app carry a real engine — **Chess Grandmaster** bundles full-strength Stockfish (NNUE), and **Offline Cheap Text LLM BitNet** runs llama.cpp (wllama) — both entirely offline in the sandbox.
- `capabilities.gpu` — **the WebGPU hatch**. WebGPU is a Permissions-Policy feature, so a sandboxed app (opaque origin) gets `navigator.gpu` only if the parent frame delegates it. Declaring `gpu` adds `webgpu` to the app frame's `allow` attribute (`runtime.js`, alongside `motion`) — set at navigation, because a permissions policy is fixed once `srcdoc` lands. Unlike `wasm` it needs **no CSP relaxation**: WebGPU opens no network path (`connect-src 'none'` still holds; nothing can appear in a URL), it drives a compute/render pipeline on the device's GPU and nothing else. It composes with `wasm` — an engine still reaches the sandbox as bytes (base64 or the install-time asset cache) — and it honours the per-app opt-out like every other capability. This is what lets an app run **on-device WebGPU inference** (transformers.js / ONNX Runtime Web / WebLLM) where the device has a real GPU behind it; where it doesn't, the engine falls back to CPU and parity is fine.
- `capabilities.ai` — the app may call `gifos.ai.chat/tts/stt/image/imageToVideo/video` and `gifos.ai.models()`. Declare it as an **array of the AI types the app actually uses** — `"ai": ["smartest","cheapest","image"]` — from `smartest`/`cheapest` (text), `tts`, `stt`, `image`, `image_to_video`, `video`; the runtime **gates calls to the declared types**, the acknowledgement lists them by their Settings label, and a missing one produces a specific prompt ("*Text → image isn't set up yet*"). A bare `true` is the legacy generic form (any type, unnamed). The user configures OpenAI-shaped endpoints + keys per type in **Settings → AI models** (stored in `localStorage`, per-origin, and excluded from a shareable backup GIF). The runtime attaches the key and returns the result; **the app never sees a key** and is portable across providers. **Streaming:** `gifos.ai.chat` takes an optional `onDelta(piece)` — pass it and the runtime asks the endpoint to stream, parses the server-sent events and hands each fragment over as it lands (a function can't cross `postMessage`, so the callback stays app-side and the runtime posts `part` messages carrying the call id). The promise still resolves **once**, with the complete `{ text, streamed }`. It degrades honestly in both directions: an endpoint that ignores `stream:true` and answers with one JSON body, and a **Provider app** (which answers in one piece), simply never call `onDelta` — so an app that renders deltas *and* paints the final `r.text` is correct on every path.
- `capabilities.api` — an **array of named third-party APIs** the app uses (`"api": ["deepgram"]`), for keyed services that aren't OpenAI-shaped. The user names each API in **Settings → Third-party APIs** with a base URL, an auth scheme (`bearer` / `token` / custom `header` / `query` param) and a key (again `localStorage`, per-origin, out of any backup GIF). The app calls `gifos.api(name, { method, path, query, headers, body, as })`; the runtime attaches the credential per the configured scheme and **pins it to the API's own origin** — a request whose resolved URL leaves that host is refused, so an app cannot redirect the key elsewhere. Same broker pattern as `ai`, generalised to any key. **Browser reachability:** these are direct `fetch`es from the page origin, so the target must return permissive CORS headers. Providers that only serve server-to-server (e.g. Deepgram's REST API sends no `Access-Control-Allow-*`) are handled by an **optional CORS proxy** the user toggles per-API in Settings: the runtime then sends the request to `cors-proxy.gifos.app` (a stateless Cloudflare Worker in `cors-proxy/`) with the true destination in an `x-gifos-target` header, and the Worker forwards it and adds the CORS headers. The proxy stores nothing, is gated to `gifos.app` origins, and only forwards to an allow-list of API hosts; users can point at their own copy (widening the allow-list) via an Advanced field. Because Workers bill by request/CPU and never by bandwidth, running it is effectively free — the metered API cost stays on the user's own key.
...
- `provides` (optional) — names a **Provider app**: an app that SERVES an AI
  role (e.g. `{ "ai": ["tts"] }`) to the whole computer once assigned in
...
- *Open rooms*: anyone runs an app; ONE app wins everywhere by a deterministic
  rule (newest ad by timestamp, peer-id tiebreak) so the room does not split;
...
+ `connect-src blob: data:` so an app can run a compiled engine (including
emscripten loaders that fetch their wasm from a self-minted blob: URL) —
compute is unlocked, but blob:/data: name no host, so the network is not.
...
- **An invite link is a capability, not a viewer pass.** A joiner's browser
  ...
  - **Lifetime** — how long the link admits *new* joiners. `close` (default) is
    ...
    admission deadline; `forever` sets no deadline. Expiry only shuts the door —
