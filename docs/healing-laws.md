...
- **R2.** The relay is a ZERO-KNOWLEDGE GREETER REGISTRY keyed by the hashed URL. It stores ONLY `H(genesis key)` and a TTL'd list of SEALED greeter entries — each `Seal(K, address)` under the meeting-URL key `K` the relay never holds, where **address = the greeter's `{peerId, coord}`** (a real address, not a bare id: an opener learns *where* the greeter sits). On knock it returns the sealed list and ADMITS the knocker iff the list is empty (mint genesis) or the presented key hashes to the stored `H(genesis key)`. Each greeter's opaque peer id IS the handle that reaches its socket, so the relay can deliver an introduction straight to a named greeter — that is fine and expected (greeters are the public front door). It holds no home, no coords, no seat-state, no names, no IPs, no room contents, and arbitrates nothing — arrival order alone decides genesis. A URL-holder decrypts the list and reaches a **randomly chosen** greeter (not always the first — the pick is spread across the whole Section-1 pool so no single greeter is hammered); the operator sees only ciphertext + a hash. Entries expire on TTL; an empty list forgets the key.
