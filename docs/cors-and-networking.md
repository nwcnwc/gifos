...
- **`api` capability** — a keyed third-party API configured in *Settings → Third-party APIs*. Test & save probes **directly first**; only if the host blocks browser calls does it fall back to the proxy (or a self-hosted proxy URL under Advanced) and remember that. The runtime attaches the credential either way (see `brokerApi`). **Deepgram bypasses all of this**: the broker speaks its WebSocket protocol natively (`deepgramListenWS` — no CORS preflight on a WS handshake, key in the subprotocol, REST-shaped JSON back), so the key and audio go straight to Deepgram and the shared proxy never sees them. The long-term aim is that the shared proxy carries only keyless public-data hosts — see roadmap §17 "keyed-traffic refusal".
