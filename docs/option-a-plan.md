...
### No cascade — the mover is always a leaf
Filling a hole never cascades down a subtree, because **only a leaf ever moves**. A
down-child that has a subtree promotes a *deep* leaf and **stays put**; it moves itself
...
- **F2 — Column-major section seating.** Fill a section breadth-first (all heads, then column 1, then column 2, …) so inter-row cross-links form as early as possible, instead of filling one row deep before the next. **CAVEAT (2026-07-18):** Section-1 admission is now ROW-major by law (healing-laws H7 row-fill) — the media plane's near field is row-scoped, so the first C people in a room MUST be row-mates (the old column-first spread seated a 2-person meeting as column-mates with zero direct media). F2, if ever built, applies to DEEP sections only, and must still keep each partially-filled row dense before opening the next (media first, cross-link earliness second).
