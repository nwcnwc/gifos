...
- **Admin rooms (serverless!).** Anyone inside an un-administered room can set an **admin password** — from then on only admins can set the room password, globally mute/blur, or **ban devices** (bans sever live media, refuse rejoins, and are undoable from the admin's banned-list panel). The admin link is `gifos.app/call/<room>/<key>` where the key is a *room-salted hash* of the admin password: the relay's occupants hold only a hash **of that hash**, so no server ever stores a password, and the relay stamps admin actions so nobody can spoof one. An admin returning to an emptied room re-seeds the ban list from their own device.
