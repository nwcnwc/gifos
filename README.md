...
- **Calls are civilized.** Everyone joins **muted, camera off, and Max-blurred** — you're invisible until you choose to be seen. Blur is three-state: **Max blur** (nothing legible), plain **Blur** (a soft silhouette), or off. Crucially it's **enforced at the sender** — when you blur yourself your browser broadcasts *already-blurred* pixels, so nobody can strip a CSS rule and recover a clear image — *and* at every receiver. Every tile shows live mute/blur/camera status; **anyone can mute or blur anyone for the whole room**, always attributed and un-liftable by the target.
- **Moderation for public rooms.** Rooms can be **password-locked** by anyone inside (occupant-held, never stored). Beyond that, two anti-abuse tools: in **admin rooms**, a **"Blur all guests"** control (three-state, defaults to plain) blurs every non-admin — so even an ungovernable-but-named room can't show clear obscenity; and in **non-admin rooms**, **vote-off-the-island** — a ✕ on each tile, and when half the room votes someone off, the relay kicks and bans that device. Mob rule, but self-limiting and the point of a public commons.
