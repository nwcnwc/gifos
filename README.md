...
- **A password is the key to clear video, period.** No room can show unblurred video unless a **room password is set** — try to turn blur off in an open room and you get *"Password must be set for unblurred video."* This is the whole child-safety story in one rule: a room a stranger can wander into can *never* be clear. A password can only be set in an **admin room** (by the admin) or in a **plain room with the current participants' consent** — so clear video always implies a locked door someone stands behind. Remove the password and the whole room re-blurs instantly.
- **On top of the password: consent, and a present moderator.** Plain rooms also need **unanimous agreement** — the room stays at least softly blurred until *every* person has turned their own blur off; one newcomer resets it. Admin rooms need an **admin actually connected** — if none is, everyone auto-blurs until one returns. And a **"Blur all guests"** control (admin, three-state) can force every non-admin blurry regardless.
