...
- **Clear video only where moderation is possible.** The rule everywhere: no clear video without someone (or everyone) accountable for it. In **plain rooms** that means **unanimous consent** — the room stays at least softly blurred until *every* person has turned their own blur off; one newcomer resets it. In **admin rooms** it means an admin: if **no admin is connected**, everyone auto-blurs until one returns. Nobody can demand others go clear while hiding themselves.
- **Moderation for public rooms.** Rooms can be **password-locked** by anyone inside (occupant-held, never stored). In **admin rooms**, a **"Blur all guests"** control (three-state, defaults to plain) blurs every non-admin. In **non-admin rooms**, **vote-off-the-island** — a ✕ on each tile, and when half the room votes someone off, the relay kicks and bans that device. Mob rule, but self-limiting and the point of a public commons.
- **Ban lists in plain rooms only ever grow.** The relay stores nothing — instead **every participant carries the room's ban list away** in their own browser and it **merges back in** whenever they return. A vote-banned troll who sneaks into a room that's forgotten them is booted the instant one person who remembers walks in. Bans outlive the room through its people.
- **Adults-only rooms (admin-set, and only there).** An admin can mark their room adults-only — joiners see one warning, optionally with a **challenge question enforced by the relay** like a password. That's the only place any such language exists; the commons never mention it.
...
- ✅ Call safety: clear video only by unanimous consent (plain rooms) or under a connected admin; grow-only community ban lists carried by participants; vote-off-the-island; admin-set adults-only gate; who-is-here address transparency
