...
(function (root) {
  ...
  var CAP_DESC = {
    ...
    screen: 'Lets the app capture a window, a browser tab, or your whole screen — for a recorder, a whiteboard, a teaching tool. <b>This is the widest thing on this list</b>, because whatever you pick is visible to the app while it runs: pick your whole screen and that includes every other window on it. Your browser, not GifOS, runs the picker and the stop button: nothing is captured until you choose a surface yourself, and you can end it from the browser’s own bar at any moment. Capturing gives the app no way to reach the internet — what it sees stays sealed inside it unless it already had a network ability you granted separately. <b>You do not need this to share your screen in a meeting</b>: the meeting does that itself, and an app running in a meeting is already on everyone’s device.',
