 ...
 * variables), a default iconSize for first-boot seeding, and optional Easter-
 * egg apps seeded only on that computer.
 *
 ...
 *     iconSize,    // default desktop icon px at first boot (32–160)
...
(function (root) {
  ...
  // ui keys map to CSS variables (see desktop.css :root). `onaccent` is the
  // text colour on accent-filled buttons — set it dark on bright/pastel
  ...
  const THEMES = {
    ...
    home: { name: 'Desktop', pack: 'aurora', iconSize: 88, ui: {
      ...
      bar: 'rgba(255,255,255,.9)', label: '#1d1d1f', labelshadow: 'rgba(255,255,255,.92)',
      onaccent: '#ffffff',
    } },

    // 0 — developers, hackers, tinkerers. Phosphor CRT.
    0: { name: 'Terminal Zero', pack: 'terminal', iconSize: 56, ui: {
    ...
    1: { name: 'Letterpress', pack: 'letterpress', iconSize: 64, ui: {
    ...
    // 2 — young kids & families. Hand-drawn kawaii stickers.
    2: { name: 'Sticker Meadow', pack: 'sticker', iconSize: 80, ui: {
    ...
    3: { name: 'Toybox', pack: 'toybox', iconSize: 76, ui: {
    ...
    4: { name: 'Stadium', pack: 'stadium', iconSize: 68, ui: {
    ...
    5: { name: 'Countdown', pack: 'countdown', iconSize: 60, ui: {
    ...
    6: { name: 'Watercolor', pack: 'watercolor', iconSize: 88, ui: {
    ...
    7: { name: 'Lucky Sevens', pack: 'sevens', iconSize: 64, ui: {
    ...
    8: { name: '8-Bit', pack: 'eightbit', iconSize: 48, ui: {
    ...
    // 9 — wellness, nature. Raked sand and ink wash. Light.
    9: { name: 'Zen Garden', pack: 'zen', iconSize: 72, ui: {
  ...
  GifOS.themeIconSize = function () {
    const n = GifOS.theme && GifOS.theme.iconSize;
    return (typeof n === 'number' && n >= 32 && n <= 160) ? n : 64;
  };
