...
const GIF = path.join(__dirname, '..', '..', '..', 'site', 'apps', 'bingo', 'bingo.gif');
...
(async () => {
  ...
  const fid = await page.evaluate(async (b64) => {
    ...
    await GifOS.store.putFile({ id: fid, name: 'bingo.gif', bytes, kind: 'gif', isApp: true, appId: 'bingo', mime: 'image/gif' });
    await GifOS.store.putItem({ id: GifOS.store.uid('item'), kind: 'file', fileId: fid, name: 'Bingo.gif', parent: null, x: 200, y: 200, iconSize: 64 });
    return fid;
  }, GIF_B64);
  ...
  await fr.waitForFunction(() => window.Bingo && window.Bingo.coverShot, null, { timeout: 20000 });
  await fr.evaluate(() => window.Bingo.coverShot());
  await fr.waitForFunction(() => {
    const n = document.getElementById('callNum');
    return n && n.textContent && n.textContent !== 'Go' && n.textContent !== '—';
  }, null, { timeout: 8000 });
  await sleep(250);
