...
var AudioEngine = {
    ...
    loadAudioFile: function (src) {
        var key = src;
        if (key.indexOf('audio/') === -1 && key.indexOf('/') !== -1) {
            var slash = key.lastIndexOf('/');
            if (slash >= 0 && key.indexOf('audio/') === -1) {
                /* keep as-is */
            }
        }
        var rel = src;
        if (rel.indexOf('audio/') === -1) {
            var m = String(src).match(/audio\/[^?#]+/);
            if (m) rel = m[0];
        }
        if (AudioEngine.AUDIO_BUFFER_CACHE[rel] || AudioEngine.AUDIO_BUFFER_CACHE[src]) {
            return new Promise(function (resolve) {
                resolve(AudioEngine.AUDIO_BUFFER_CACHE[rel] || AudioEngine.AUDIO_BUFFER_CACHE[src]);
            });
        }
        var loader = window.__adrLoadAudio;
        var bytesP = loader ? loader(rel) : Promise.reject(new Error('no audio loader'));
        return bytesP.then(function (buffer) {
            ...
            var decodeAudioDataPromise = AudioEngine._audioContext.decodeAudioData(buffer, function (decodedData) {
                AudioEngine.AUDIO_BUFFER_CACHE[rel] = decodedData;
                AudioEngine.AUDIO_BUFFER_CACHE[src] = decodedData;
                return decodedData;
            });
            if (decodeAudioDataPromise) {
                return decodeAudioDataPromise.then(function (decoded) {
                    AudioEngine.AUDIO_BUFFER_CACHE[rel] = decoded;
