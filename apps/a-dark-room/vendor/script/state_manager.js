...
var StateManager = {
	...
	_tokens: function(stateName) {
		var words = String(stateName).split(/[.\[\]'"]+/);
	...
	_read: function(stateName) {
		var words = $SM._tokens(stateName);
	...
	_write: function(stateName, value) {
		var words = $SM._tokens(stateName);
		...
		for (var i = 0, len = words.length - 1; i < len; i++) {
			w = words[i];
			if (obj[w] === undefined || typeof obj[w] !== 'object') obj[w] = {};
			obj = obj[w];
		}
	...
	get: function(stateName, requestZero) {
		var whichState = $SM._read(stateName);
	...
	setget: function(stateName, value, noEvent){
		...
		return $SM._read(stateName);
	},

	remove: function(stateName, noEvent) {
		$SM.buildPath(stateName);
		try{
			$SM._delete(stateName);
		} catch (e) {
