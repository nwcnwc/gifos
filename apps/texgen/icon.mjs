...
function buildPalette() {
  const pal = [[0, 0, 0]];
  for (const b of [CARD, A, B, C, [255, 255, 255], [20, 24, 32]]) {
...
function tex(x, y, f) {
  const xor = ((x ^ y) & 255) / 255;
  const sx = 0.5 + 0.5 * Math.sin((x + f * 6) * 0.18);
  const sy = 0.5 + 0.5 * Math.sin((y + f * 4) * 0.16);
  return mix(mix(A, B, xor), C, sx * 0.35 + sy * 0.2);
}
