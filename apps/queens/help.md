...
A logic puzzle. Place **N queens** on an N×N board so that:

- every **row** has exactly one queen
- every **column** has exactly one queen
- every **coloured region** has exactly one queen
- no two queens **touch at a corner** (diagonally adjacent)

Queens do not attack down a whole diagonal the way they do in chess — only the four neighbouring corners count.

## Controls

**Tap** a cell:

1. empty → **X** (this square is not a queen)
2. X → **queen**
3. queen → empty
...
Finished level numbers, Auto X, and the board you were in the middle of stay in this file. A friend’s marks live on their device.
