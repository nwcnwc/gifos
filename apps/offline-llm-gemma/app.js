...
(function () {
  ...
  function chatHandler(req, ctx) {
    ...
    return bootEngine(beat).then(function (wllama) {
      ...
      var cleanUp = function (raw) { return String(raw).split(END)[0].split('<start_of_turn>')[0].replace(/^\s+/, ''); };
      var acc = '';
      params.stream = true;
      var tok = 0;
      ...
      var label = live.kind === 'selftest'
        ? '[self-test model — token soup, not intelligence. Install the Gemma 3 weights for real answers.]\n' : '';
