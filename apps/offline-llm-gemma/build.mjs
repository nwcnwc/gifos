// Pack apps/offline-llm-gemma/ source into the downloadable
// site/apps/offline-llm-gemma/offline-llm-gemma.gif (see apps/README.md).
//
...
const bytes = await gif.encode(files, { preview: gemmaIcon(), accent: manifest.accent });
const out = join(dir, '..', '..', 'site', 'apps', 'offline-llm-gemma', 'offline-llm-gemma.gif');
mkdirSync(dirname(out), { recursive: true });
writeFileSync(out, bytes);
console.log('wrote site/apps/offline-llm-gemma/offline-llm-gemma.gif —', (bytes.length / 1e6).toFixed(2), 'MB from', Object.keys(files).length, 'files (engine + self-test model in-GIF; Gemma weights by asset pin)');
