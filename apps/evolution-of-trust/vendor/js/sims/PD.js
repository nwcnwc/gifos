...
PD.playOneGame = function(playerA, playerB){
	...
	var rnd = (window.TRUST && TRUST.random) ? TRUST.random : Math.random;
	if(rnd()<PD.NOISE) A = ((A==PD.COOPERATE) ? PD.CHEAT : PD.COOPERATE);
	if(rnd()<PD.NOISE) B = ((B==PD.COOPERATE) ? PD.CHEAT : PD.COOPERATE);
...
function Logic_random(){
	...
	self.play = function(){
		var rnd = (window.TRUST && TRUST.random) ? TRUST.random : Math.random;
		return (rnd()>0.5 ? PD.COOPERATE : PD.CHEAT);
	};
	self.remember = function(own, other){
		// nah
	};
