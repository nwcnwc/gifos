...
function Slider(config){
	...
	var _mouseToParam = function(event){
		...
		// Mouse to Param to Value (use painted rect so CSS scale on a phone still aims)
		var rect = dom.getBoundingClientRect();
		var pad = rect.width * 8 / Math.max(1, config.width);
		var param = (event.clientX - _offsetX - rect.left - pad)/Math.max(1, rect.width - pad*2);
		if(param<0) param=0;
		if(param>1) param=1;
