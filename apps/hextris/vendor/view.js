...
function drawPlayTriangle(x, y, size, color) {
	ctx.save();
	ctx.fillStyle = color;
	ctx.beginPath();
...
function renderText(x, y, fontSize, color, text, font) {
	ctx.save();
	if (!font) {
		var font = 'px "Trebuchet MS", "Segoe UI", sans-serif';
	}
...
function drawScoreboard() {
	...
	if (gameState === 0) {
		drawPlayTriangle(trueCanvas.width / 2 + gdx, trueCanvas.height / 2 + gdy, 28 * settings.scale, "rgb(236, 240, 241)");
