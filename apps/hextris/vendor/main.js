/* GifOS port of Hextris. Original (C) 2014–2018 Logan Engstrom et al.
   GNU GPL v3 or later. Modified 2026: no score phone-home, no remote a.js,
   hexHistory instead of window.history. */
...
function init(b) {
	...
	$("#pauseBtn").attr('src',"./images/btn_pause.svg");
	hideUIElements();
	var saveState = (window.HT && HT.lsGet ? HT.lsGet("saveState") : null) || "{}";
...
function addNewBlock(blocklane, color, iter, distFromHex, settled) { //last two are optional parameters
	...
	if (!hexHistory[MainHex.ct]) {
		hexHistory[MainHex.ct] = {};
	}

	hexHistory[MainHex.ct].block = {
		blocklane: blocklane,
		color: color,
		iter: iter
	};
	...
	if (distFromHex) {
		hexHistory[MainHex.ct].distFromHex = distFromHex;
	}
	if (settled) {
...
function exportHistory() {
	$('#devtoolsText').html(JSON.stringify(hexHistory));
	toggleDevTools();
}
