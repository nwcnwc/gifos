/* GifOS port of Hextris. Original (C) 2014–2018 Logan Engstrom et al.
   GNU GPL v3 or later. Modified 2026: boot.js calls initialize after
   hydrating scores; no analytics; hexHistory instead of window.history. */
function initialize(a) {
	...
	window.op = 0;
	window.saveState = (window.HT && HT.lsGet ? HT.lsGet("saveState") : null) || "{}";
	if (saveState !== "{}") {
		op = 1;
	}
	...
	window.settings;
	if (/Android|webOS|iPhone|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) || (window.HT && HT.phoneish && HT.phoneish())) {
		settings = {
			os: "other",
			platform: "mobile",
			startDist: 227,
	...
	highscores = [];
	if (window.HT && HT.lsGet && HT.lsGet('highscores')) {
		try {
			highscores = JSON.parse(HT.lsGet('highscores'));
		} catch (e) {
			highscores = [];
		}
	}
