// Procedural hexahedral icon: a green cube stepping onto a pink block.
import { deflateSync } from 'node:zlib';

const OUT = 128, SS = 3, RW = OUT * SS, FRAMES = 12;
const FELT = [18, 16, 20], GOLD = [211, 45, 143], GOLD2 = [0, 179, 197];
const INK = [26, 18, 8], RED = [211, 45, 143], CREAM = [204, 182, 160];
const DARK = [10, 10, 15];
...
function frameIndices(pal, f) {
  ...
  for (let py = 0; py < RW; py++) for (let px = 0; px < RW; px++) {
    ...
    if (inRR(x, y, 24, 70, 64, 110, 6)) col = GOLD2;
    if (inRR(x, y, 66, 70 - drop, 106, 110 - drop, 6)) col = mix(GOLD, [255, 120, 180], 0.2);
    if (inRR(x, y, 40 + t * 28, 40, 70 + t * 28, 70, 5)) col = [110, 189, 75];
    const o = (py * RW + px) * 4;
...
export function screenshotPng() {
  ...
  rr(40, 30, 1160, 690, 24, 24, 20, 26);
  drawText(put, 64, 50, 'HEXAHEDRAL', 7, 244, 236, 224);
  drawText(put, 64, 128, 'THIRTY JAM LEVELS', 3, 211, 45, 143);
  for (let r = 0; r < 4; r++) for (let c = 0; c < 4; c++) {
    const x0 = 80 + c * 140 + r * 20, y0 = 220 + r * 80;
    const pink = (r + c) % 2 === 0;
    rr(x0, y0, x0 + 120, y0 + 90, 10, pink ? 211 : 0, pink ? 45 : 179, pink ? 143 : 197);
  }
  rr(520, 300, 640, 420, 12, 110, 189, 75);
  drawText(put, 700, 280, 'PUSH THEM DOWN', 4, 204, 182, 160);
  drawText(put, 700, 360, 'TAP OR SWIPE', 4, 0, 179, 197);
  drawText(put, 64, 630, 'THE FILE IS THE SAVE', 3, 244, 236, 224);
