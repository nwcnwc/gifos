...
export class Cube extends Group {
    ...
    /** Called after a drag snaps to a 90° turn. turns is 0 when the layer snaps back. */
    public onTurn: ((turns: number, finish: boolean) => void) | null = null;
    public get squares() {
    ...
    public getAfterRotateAnimation() {
        ...
        let rotateTick = (tick: number): boolean => {
            ...
            if (rotatedAngle < Math.abs(needRotateAnglePI)) {
                ...
                const turns = Math.round(this.state.rotateAnglePI / (Math.PI * 0.5));
                this.updateStateAfterRotate();
                if (this.onTurn) this.onTurn(turns, this.finish);
                return false;
            }
        }
     ...
     */
    public disorder() {
        this.scramble(((Math.random() * 0x100000000) >>> 0));
    }

    /**
     * Rotate one layer of the cube immediately (no animation). axis is a unit
     * world axis, layer is 0..order-1, quarterTurns is 1, 2 or 3.
     */
    public rotateLayer(axis: Vector3, layer: number, quarterTurns: number) {
        const size = this.data.elementSize;
        const border = (this.order * size) / 2 - 0.5;
        const layerCoord = -border + layer * size;
        const angle = quarterTurns * Math.PI * 0.5;
        const mat = new Matrix4().makeRotationAxis(axis.clone().normalize(), angle);
        for (let i = 0; i < this.data.elements.length; i++) {
            const el = this.data.elements[i];
            const temPos = el.pos.clone().add(el.normal.clone().normalize().multiplyScalar(-0.5 * size));
            if (Math.abs(temPos.dot(axis) - layerCoord) > 0.15) continue;
            el.pos.applyMatrix4(mat);
            el.normal.applyMatrix4(mat);
            el.pos.x = Math.round(el.pos.x * 2) / 2;
            el.pos.y = Math.round(el.pos.y * 2) / 2;
            el.pos.z = Math.round(el.pos.z * 2) / 2;
            el.normal.x = Math.round(el.normal.x);
            el.normal.y = Math.round(el.normal.y);
            el.normal.z = Math.round(el.normal.z);
        }
    }

    /**
     * Deterministic scramble. Same seed + order → the same mixed cube, which is
     * what lets a race share one shuffle without sending the cube itself.
     */
    public scramble(seed: number) {
        this.data.initialFinishData();
        const rng = mulberry32(seed >>> 0);
        const n = this.order;
        const count = n <= 2 ? 12 : n === 3 ? 25 : n * 8;
        ...
        for (let i = 0; i < count; i++) {
            ...
            const turns = 1 + ((rng() * 3) | 0);
            this.rotateLayer(axes[axis], layer, turns);
            lastAxis = axis;
            lastLayer = layer;
        }
        this.createChildrenByData();
