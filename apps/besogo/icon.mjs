// Procedural icon: a wooden go board with a black stone settling onto a
// 4-4 star point. Pure Node, super-sample → box-downsample. Deterministic.
...
function frameIndices(pal, f) {
  ...
  const stoneR = 6.4;
  // 4-4 star points (classic opening), plus a neighbour
  const br = boardIn + 2 * step, bc = boardIn + 2 * step;
  const wr = boardIn + 6 * step, wc = boardIn + 6 * step;
  const by = br - drop * 14;
...
export function screenshotPng() {
  ...
  const N = 13, pad = 36, span = board - 2 * pad, step = span / (N - 1);
  ...
  [[3, 3], [3, 9], [9, 3], [9, 9], [6, 6]].forEach(function (p) { star(p[0], p[1]); });
  ...
  // A short 4-4 opening that reads as Go, not five-in-a-row
  const blacks = [[3, 3], [9, 9], [2, 5], [10, 6], [5, 2]];
  const whites = [[3, 9], [9, 3], [5, 10], [6, 2]];
  blacks.forEach(function (p, i) { stone(p[0], p[1], true, i === blacks.length - 1); });
  whites.forEach(function (p) { stone(p[0], p[1], false, false); });

  drawText(put, 740, 150, 'BESOGO', 9, 232, 196, 122);
  drawText(put, 740, 250, 'GO BOARD', 4, 196, 154, 96);
  drawText(put, 740, 360, 'TWO SEATS', 3, 232, 220, 200);
  drawText(put, 740, 410, 'TAP TO PLACE', 3, 232, 220, 200);
