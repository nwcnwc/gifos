(function (global) {
  'use strict';
  var console = { log: function () {}, time: function () {}, timeEnd: function () {}, warn: function () {}, error: function () {} };
  function randomBytes(n) {
    var buf = new Uint8Array(n);
    var c = global.crypto;
    if (c && c.getRandomValues) c.getRandomValues(buf);
    else for (var i = 0; i < n; i++) buf[i] = Math.floor(Math.random() * 256);
    return { readUInt8: function (i) { return buf[i]; } };
  }
  var crypto = { randomBytes: randomBytes };
...
Utils.loadRule = function (ruleName) {
  var rule = (global.BG && global.BG.casual) ? global.BG.casual : null;
  if (rule) rule.name = Utils.sanitizeName(ruleName || 'RuleBgCasual');
  return rule;
};
