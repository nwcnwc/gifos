/**
 * Snarky end-of-game copy — English from maxwellito/breaklock src/l10n/en.yml
 * plus the range picker in src/controllers/summary/summaryFeedback.js.
 */
(function (root) {
  'use strict';
  ...
  function getQuote(wasSuccess, attemptsCount) {
    var feedback, matches, i, q;
    if (wasSuccess) {
      feedback = 'Lock found in ' + attemptsCount + ' ' + pluralAttempts(attemptsCount) + '.';
      matches = [];
      for (i = 0; i < SUCCESS_QUOTES.length; i++) {
        q = SUCCESS_QUOTES[i];
        if (q.min <= attemptsCount && q.max >= attemptsCount) matches.push(q.text);
      }
      if (!matches.length) matches = ['No comment.'];
    } else {
      feedback = "Sorry, you didn't make it this time.";
      matches = FAIL_QUOTES;
    }
    return feedback + ' ' + pick(matches);
  }

  root.BreakLockQuotes = {
    getQuote: getQuote,
    SUCCESS_QUOTES: SUCCESS_QUOTES,
    FAIL_QUOTES: FAIL_QUOTES
  };
