/*
 * Radius Raid — twin-stick for a phone.
 *
 * Upstream is WASD + mouse. On a phone it renders and you cannot move or aim.
 * Left pad is move (a rate), right pad is aim (a direction) and fires while
 * the stick is held out — the same layout Geometry Wars uses on a thumb.
 *
 * Shown only after a real touchstart, and only during play. A laptop with a
 * touchscreen reports ontouchstart and must not get a phone HUD over WASD.
 */
(function (root) {
  'use strict';
  ...
  function init() {
    ...
    bindStick(el.move, 'move');
    bindStick(el.aim, 'aim');
    if (el.pause) {
      el.pause.addEventListener('pointerdown', function (e) {
        if (e.pointerType === 'mouse') return;
        e.preventDefault();
        e.stopPropagation();
        if (root.$ && $.keys && $.keys.state) $.keys.state.p = 1;
      });
      el.pause.addEventListener('pointerup', function () {
        if (root.$ && $.keys && $.keys.state) $.keys.state.p = 0;
      });
    }
