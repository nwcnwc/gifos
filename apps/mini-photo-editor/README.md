...
(MIT) by xdadda. Crop, rotate, filter a photo on this device. The
upstream UI is mini-js + mini-gl; this wrap keeps the crop / rotate /
...
`db` + `multiplayer`. `minBuild` **947**. No network. Invite shares the
recipe (sliders + look), not the photo.
