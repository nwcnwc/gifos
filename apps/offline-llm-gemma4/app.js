...
(function () {
  ...
  function chatHandler(req, ctx) {
    ...
    return bootEngine(beat).then(function (wllama) {
      ...
      var cleanUp = function (raw) { return String(raw).split(T_CLOSE)[0].split(T_OPEN)[0].replace(/^\s+/, ''); };
      var acc = '';
      params.stream = true;
      var tok = 0;
      ...
      var label = live.kind === 'selftest'
        ? '[self-test model — token soup, not intelligence. Install the Gemma 4 weights for real answers.]\n' : '';
