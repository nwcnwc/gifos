// Procedural QR Code sticker: a white rounded card, finder squares that
// fill in, modules that spark. Super-sample → box-downsample.
import { deflateSync } from 'node:zlib';

const OUT = 128, SS = 3, RW = OUT * SS, FRAMES = 12;
const CARD = [250, 251, 253];
const DOT = [18, 18, 18];
const GOLD = [240, 196, 32];
const INK = [28, 36, 52];
...
function frameIndices(pal, f) {
  ...
  for (let py = 0; py < RW; py++) for (let px = 0; px < RW; px++) {
    ...
    let col = CARD;
    const qx = 22, qy = 22, qs = 4.2;
    const q = finder(x, y, qx, qy, qs) || finder(x, y, qx + 52, qy, qs) ||
      finder(x, y, qx, qy + 52, qs);
    if (q && t > 0.12) col = q === DOT ? mix(DOT, GOLD, t < 0.35 ? 0.35 : 0) : CARD;
    const mx = Math.floor((x - 24) / 4.2), my = Math.floor((y - 24) / 4.2);
    if (mx >= 0 && my >= 0 && mx < 20 && my < 20 && t > 0.4) {
      const inF = (mx < 8 && my < 8) || (mx >= 12 && my < 8) || (mx < 8 && my >= 12);
      if (!inF && ((mx * 7 + my * 13 + mx * my + f) % 3 === 0)) col = DOT;
    }
    const o = (py * RW + px) * 4;
...
export function screenshotPng() {
  ...
  function chip(x, y, w, label, on) {
    if (on) rr(x, y, x + w, y + 44, 18, 240, 196, 32);
    else rr(x, y, x + w, y + 44, 18, 22, 22, 32);
    const tw = String(label).length * 18;
    drawText(put, x + Math.floor((w - tw) / 2), y + 12, label, 3, on ? 22 : 244, on ? 18 : 241, on ? 10 : 232);
  }
  chip(600, 196, 150, 'TEXT', false);
  chip(764, 196, 150, 'LINK', true);
  chip(928, 196, 168, 'PHONE', false);

  rr(600, 268, 1136, 400, 10, 16, 16, 24);
  drawText(put, 624, 318, 'HTTPS://GIFOS.APP', 3, 244, 241, 232);

  drawText(put, 600, 450, 'LAST CODE STAYS IN THIS FILE', 3, 154, 148, 134);
  drawText(put, 600, 510, 'INVITE SHOWS THE SAME CODE', 3, 154, 148, 134);
  drawText(put, 600, 600, 'UNOFFICIAL PORT OF QRCODEJS', 3, 240, 196, 32);
