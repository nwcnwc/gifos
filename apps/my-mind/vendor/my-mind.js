...
${text}`);
  ...
  function init11() {
    [LocalUI, FileUI, ImageUI].forEach((ctor) => {
      let bui = new ctor();
  ...
  var _ready = window.MyMind && window.MyMind.onReady;
  window.MyMind = {
    getJSON: function () { return currentMap && currentMap.toJSON(); },
    loadJSON: function (j) { showMap(Map2.fromJSON(j)); },
    newMap: function () { showMap(new Map2()); },
    subscribe: subscribe
  };
  if (_ready) window.MyMind.onReady = _ready;
  init19().then(function () {
    if (window.MyMind.onReady) window.MyMind.onReady();
  });
})();
