...
if (!fleet) {
  ...
  //
  // THERE IS NO BASELINE, AND THERE MUST NEVER BE ANOTHER ONE. This check
  // shipped scoped to BASELINE..HEAD because 73 messages older than it were
  // already public and only a history rewrite could fix them — a red nobody
  // could act on. On 2026-08-25 that rewrite happened: 2374 commits across 42
  // branches and 11 tags were rewritten and force-pushed, every tree
  // byte-identical, and the count went to zero. So the assertion now covers
  // EVERY commit reachable from every branch and tag.
  //
  // If this ever goes red again, the fix is the message, not the range. A red
  // here is either (a) a message you have not pushed yet, where `git commit
  // --amend` or `git rebase -i <sha>^` costs a minute, or (b) a message that IS
