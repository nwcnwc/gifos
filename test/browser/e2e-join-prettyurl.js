...
// for an owned one — not the internal meet.html#j=…/#s=… form it loads via. We
...
(async () => {
  ...
  await p1.goto(base + '/meet.html#j=' + code);
  ...
  check('no meet.html#j= left in the address bar',
    !(await p1.evaluate(() => location.href)).includes('meet.html#j'));
  ...
  p2.on('pageerror', () => {});
  await p2.goto(base + '/meet.html#s=' + room + '.' + ver + '&k=' + lsec);
  ...
  p3.on('pageerror', () => {});
  await p3.goto('http://127.0.0.1:' + PORT + '/meet.html#j=' + code);
  // give the client a moment to (not) rewrite
  await p3.waitForFunction(() => window.GifOS && window.GifOS.links, null, { timeout: 8000 });
  check('on localhost the bar stays the meet.html# form (no /join routing)',
    (await p3.evaluate(() => location.pathname)) === '/meet.html');
