...
(async () => {
  ...
  check('captured photo deposits a Camera image into My Media', photo.n >= 1, JSON.stringify(photo));
  ...
  if (info && info.video) {
    await ackPerms();
    fr.locator('body').evaluate(() => gifos.camera({ mode: 'video' }).then(() => 'ok', (e) => String(e && e.message || e))).catch(() => {});
    await page.locator('[data-gifos-studio]').waitFor({ timeout: 10000 });
    await page.locator('[data-mode="video"].on').waitFor({ timeout: 8000 }).catch(() => {});
    await page.waitForFunction(() => {
      const v = document.querySelector('[data-gifos-studio] video');
      return !!(v && v.videoWidth);
    ...
    await sleep(300);
    await page.locator('[data-cs="shutter"]').click();
    await page.locator('.cs-shutter.rec').waitFor({ timeout: 5000 }).catch(() => {});
    await sleep(1200);
    await page.locator('[data-cs="shutter"]').click();
    await page.locator('[data-cs="last"].on').waitFor({ timeout: 10000 }).catch(() => {});
    await page.locator('[data-cs="close"]').click();
    await page.locator('[data-gifos-studio]').waitFor({ state: 'detached', timeout: 8000 }).catch(() => {});
    await sleep(800);
    const vid = await page.evaluate(async (mmId) => {
      ...
      return { n: vids.length, mime: vids[0] && vids[0].mime };
    }, layout.mmId);
    if (vid.n >= 1) check('video mode deposited a type:video item in My Media', true, JSON.stringify(vid));
    else console.log('  (no video deposit on the fake device; photo deposit already passed) ' + JSON.stringify(vid));
  }
